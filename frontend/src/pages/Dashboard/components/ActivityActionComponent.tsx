...
interface PropsInterface {
  ...
  handleActivityClick: (aVal: activitiesResponse) => void;
  aVal: activitiesResponse;
}

const ActivityActionComponent = (props: PropsInterface) => {
  const { permissionArray, handleActivityClick, aVal } = props;
  return (
    <>
      {' '}
      <Tippy
            ...
            <ul className="tippy__dropdown__ul">
              {permissionArray.length > 0 &&
                (permissionArray || []).map((val) => {
                  return (
                    <li className="item" key={window.self.crypto.randomUUID()}>
                      <div className="item__link" onClick={val.onClick}>
      ...
      >
        <button
          className="toggleBtn"
          type="button"
          onClick={() => handleActivityClick(aVal)}
        >
          <Icon
...
export default ActivityActionComponent;
