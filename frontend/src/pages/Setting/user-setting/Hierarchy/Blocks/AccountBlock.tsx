...
const AccountBlock = (props: Props) => {
  ...
  const {
    ...
    watch,
    setValue,
  } = useForm<AddAccountFormFieldType>({
    resolver: yupResolver(hierarchyAccountSchema),
  });

  // ** State **
  const [isEditing, setIsEditing] = useState<keyof AddAccountFormFieldType>();
  ...
  const onSave = (key: keyof AddAccountFormFieldType) => {
  ...
  return (
    <>
      ...
      >
        <div className="header__details  !bg-[#F1F0F8]">
          <InlineEditable
            ...
            onSave={onSave('title')}
            editComponent={
              <FormField<AddAccountFormFieldType>
                type="text"
                name="title"
