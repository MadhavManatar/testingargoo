...
type AddSubDepartmentFormFieldType = {
  title: string;
  description: string;
...
const SubDepartmentBlock = (props: Props) => {
  ...
  const {
    ...
    watch,
    setValue,
  } = useForm<AddSubDepartmentFormFieldType>({
  ...
  const onSave = (key: keyof AddSubDepartmentFormFieldType) => {
  ...
  const insertNodeOptions = [
    {
      title: LEVEL_TITLE.ADD_SUB_DEPARTMENT,
      type: LEVEL_TYPES.SUB_DEPARTMENT,
      permission: true,
    },
  ...
  return (
    <>
      ...
      >
        <div className="header__details !bg-[#F7CED5]">
          <InlineEditable
            ...
            onSave={onSave('title')}
            editComponent={
              <FormField<AddSubDepartmentFormFieldType>
                type="text"
                name="title"
...
export default SubDepartmentBlock;
