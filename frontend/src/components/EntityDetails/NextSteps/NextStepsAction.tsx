...
import ToggleButtonVertical from 'assets/custom-svg-icons/ToggleButtonVertical';

// ** components **
import React from 'react';

type NextStepsActionButtonProps = {
  filedArray: { label: string; onClick: () => void; Icon?: JSX.Element }[];
  disabled?: boolean;
  iIconColor: string;
};

export const NextStepsAction = (props: NextStepsActionButtonProps) => {
  const { filedArray, disabled = false, iIconColor } = props;
  return (
    <>
      {!disabled && (
        <Tippy
        ...
        >
          <button
            ...
            className="i__Button secondary__Btn ag__grid__toggle__btn"
          >
            <div className="i__Icon">
              <ToggleButtonVertical fill={iIconColor} />
            </div>
          </button>
        </Tippy>
      )}
